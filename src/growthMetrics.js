...
async function logGrowthEvent(eventType, data) {
  ...
  logger.info(`[GROWTH_METRICS] 📈 GROWTH EVENT: ${logEntry}`);
  ...
  try {
    ...
    logger.error('[GROWTH_METRICS] Error storing growth event:', error);
  }
}
...
async function getRecentGrowthEvents(limit = 20) {
  try {
    ...
    logger.error('[GROWTH_METRICS] Error getting growth events:', error);
    return [];
  }
}
