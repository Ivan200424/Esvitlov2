...
const logger = require('./logger').child({ module: 'growth-metrics' });
...
async function logGrowthEvent(eventType, data) {
  ...
  try {
    ...
    logger.error({ err: error }, 'Error storing growth event');
  }
}
...
async function getRecentGrowthEvents(limit = 20) {
  try {
    ...
    logger.error({ err: error }, 'Error getting growth events');
    return [];
  }
}
