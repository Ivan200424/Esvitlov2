...
const logger = require('./logger').child({ module: 'admin-router-monitor' });
...
async function monitorAdminRouter(adminRouter) {
  ...
  try {
    ...
    logger.error({ err: error }, `Error monitoring admin router for ${admin_telegram_id}`);
  }
}
...
async function sendStateChangeNotification(adminTelegramId, newState, durationMinutes, routerIp) {
  try {
    ...
    logger.error({ err: error }, `Error sending state change notification to ${adminTelegramId}`);
  }
}
...
async function runMonitoringCheck() {
  try {
    ...
    logger.error({ err: error }, 'Error in admin router monitoring check');
  }
}
...
async function forceCheckAdminRouter(adminTelegramId) {
  try {
    ...
    logger.error({ err: error }, `Error force checking admin router for ${adminTelegramId}`);
    return null;
  }
}
