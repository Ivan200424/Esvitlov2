...
const logger = require('../logger').child({ module: 'pending-channels' });
...
async function savePendingChannel(channelId, channelUsername, channelTitle, telegramId) {
  try {
    ...
    logger.error({ err: error }, `Error saving pending channel ${channelId}`);
    return false;
  }
}
...
async function getPendingChannel(channelId) {
  try {
    ...
    logger.error({ err: error }, `Error getting pending channel ${channelId}`);
    return null;
  }
}
...
async function deletePendingChannel(channelId) {
  try {
    ...
    logger.error({ err: error }, `Error deleting pending channel ${channelId}`);
    return false;
  }
}
...
async function getAllPendingChannels() {
  try {
    ...
    logger.error({ err: error }, 'Error getting all pending channels');
    return [];
  }
}
