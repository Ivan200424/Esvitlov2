...
async function safeSendMessage(bot, chatId, text, options = {}) {
  try {
    ...
    if (isTelegramUserInactiveError(error)) {
      logger.info(`ℹ️ Користувач ${chatId} недоступний — повідомлення не відправлено`);
    } else {
...
async function safeEditMessage(bot, chatId, messageId, text, options = {}) {
  try {
    ...
    if (isTelegramUserInactiveError(error)) {
      logger.info(`ℹ️ Користувач ${chatId} недоступний — редагування повідомлення пропущено`);
    } else {
...
async function safeSendPhoto(bot, chatId, photo, options = {}, _fileOpts = {}) {
  try {
    ...
    if (isTelegramUserInactiveError(error)) {
      logger.info(`ℹ️ Користувач ${chatId} недоступний — відправку фото пропущено`);
    } else {
