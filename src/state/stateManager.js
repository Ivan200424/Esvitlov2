...
const logger = require('../logger').child({ module: 'state-manager' });
  ...
  for (const stateType of stateTypes) {
    ...
    for (const { telegram_id, state_data } of stateRecords) {
      try {
        ...
        logger.error({ err: error }, `Error restoring ${stateType} state for ${telegram_id}`);
      }
    }
  }

  logger.info(`✅ Restored ${totalRestored} persisted states`);
}
