...
async function handlePowerStateChange(user, newState, oldState, userState, _originalChangeTime = null) {
  try {
    ...
    try {
      ...
      logger.error({ err: error }, 'Error checking schedule');
    }
      ...
      if (notifyTarget === 'bot' || notifyTarget === 'both') {
        try {
          ...
          if (isTelegramUserInactiveError(error)) {
            ...
            logger.error({ err: error }, `Помилка відправки повідомлення користувачу ${user.telegram_id}`);
          }
          if (metricsCollector) {
        ...
        if (user.channel_paused) {
          ...
          try {
            ...
            if (isTelegramUserInactiveError(error)) {
              ...
              logger.error({ err: error }, `Помилка відправки повідомлення в канал ${user.channel_id}`);
            }
            if (metricsCollector) {
    ...
    logger.error({ err: error }, 'Error handling power state change');
  }
}
